...
function go(step) {
	...
	if (isOp) { //hallvord
        ...
        // $("#body").css({"top": "-900px"});
        $(ne).css({"top": "-900px", "zIndex" : "9999"});
        $(ce).css({"zIndex" : "0"});
		ne.style.visibility = 'visible';
        // $("#body").animate({"top": "+=900px"}, 1500, "easeOutBounce");
        $(ne).animate({"top": "+=900px"}, 1500, "easeOutBounce");
	} // /hallvord
...
function toggle() {
	...
	if (!slides.disabled) {
		...
		for (var n = 0; n < smax; n++) {
            ...
            $(slide).slideDown("slow", "easeOutBounce", function() {
});
...
function fontScale() {  // causes layout problems in FireFox that get fixed if browser's Reload is used; same may be true of other Gecko-based browsers
	...
	if (window.innerHeight) {
		...
		var vSize = 900;  // assuming 1024x768, minus chrome and such
		var hSize = 1440; // these do not account for kiosk mode or Opera Show
	}
